...

{
  ...
  $.fn.actions = function (opts) {
    ...
    const clearAcross = function () {
      reset()
      $(options.acrossInput).val(0)
      $(options.actionContainer).removeClass(options.selectedClass)
    }
    const checker = function (checked) {
      if (checked) {
        showQuestion()
      } else {
        reset()
    ...
    $('a', options.acrossQuestions).on('click', (event) => {
      event.preventDefault()
      $(options.acrossInput).val(1)
      showClear()
    })
    $('a', options.acrossClears).on('click', (event) => {
      event.preventDefault()
    ...
    lastChecked = null
    $(actionCheckboxes).on('click', (event) => {
    ...
    $('form#changelist-form table#result_list tr').on('change', 'td:gt(0) :input', () => {
      list_editable_changed = true
    })
    $('form#changelist-form button[name="index"]').on('click', (event) => {
      if (list_editable_changed) {
        return confirm(gettext('You have unsaved changes on individual editable fields. If you run an action, your unsaved changes will be lost.'))
      }
    })
    $('form#changelist-form input[name="_save"]').on('click', (event) => {
