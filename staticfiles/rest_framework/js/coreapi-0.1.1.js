      ...
      const HTTPTransport = (function () {
        ...
        _createClass(HTTPTransport, [{
          ...
          value: function action (link, decoders) {
            ...
            return this.fetch(request.url, request.options).then((response) => {
              if (response.status === 204) {
                return
              }
              return parseResponse(response, decoders, responseCallback).then((data) => {
                if (response.ok) {
                  return data
                }
